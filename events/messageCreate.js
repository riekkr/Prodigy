...
module.exports = async (client, message) => {
    ...
    if (!command) {
        if (config.unknownCommandMessage) {
            const unknownCommandEmbed = new Discord.MessageEmbed()
                ...
                .setColor('RED');
            message.channel.send({ embeds: [unknownCommandEmbed] });
        } else {
            return;
        }
    }
    ...
    if (command.requiredPermissions.length > 0) {
        ...
        for (let i = 0; i < arrLen; i++) {
            if (!message.member.permissions.has(command.requiredPermissions[i]) && !client.config.owners.includes(message.author.id)) {
                const embed = new Discord.MessageEmbed()
                    .setAuthor('Error')
    ...
    if (!dj) {
        message.reply('Created initial configuration for this server. **Run the command again.**');
